...
export default function QuizCard({ 
  ...
  return (
    <div className={`${horizontal ? 'flex gap-2 sm:gap-3 justify-center' : 'space-y-2 sm:space-y-3'} ${className}`}>
      {options.map((option, index) => (
        <button
          key={index}
          onClick={() => handleSelect(option)}
          className={`${horizontal ? 'w-32 sm:w-36' : 'w-full'} p-3 sm:p-4 rounded-lg border-2 transition-all duration-200 ${horizontal ? 'text-center' : 'text-left'} ${
            isSelected(option)
              ? 'border-[#0066FF] bg-[#E5F0FF] text-[#001433]'
              : 'border-[#0066FF]/20 bg-white text-[#003D99] hover:border-[#0066FF]/40 hover:bg-[#E5F0FF]'
          }`}
        >
          <div className={`flex items-center ${horizontal ? 'justify-center' : 'justify-between'}`}>
            <span className="text-xs sm:text-sm md:text-base font-medium">{option}</span>
            {isSelected(option) && !horizontal && (
